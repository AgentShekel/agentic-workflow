...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-ux-methodology`, `brand-methodology`, `design-system-methodology`, and `chrome-devtools` skills.
...
Present ideas to the user in plain chat for approval before designing (do NOT use `AskUserQuestion` — globally banned).

### Step 4: Design HTML Files

Activate these skills in sequence:

1. **`brand-methodology`** skill — Extract brand colors, fonts, voice from user's project
2. **`design-system-methodology`** skill — Get design tokens (spacing, typography scale, color palette)
3. **`ui-ux-methodology`** skill — for layout, hierarchy, visual balance.
