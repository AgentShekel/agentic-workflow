...
def generate_with_nano_banana(prompt_data, output_dir=None, model_key="flash", aspect_ratio="1:1", logo_image=None):
    ...
    print(f"   Brand: {prompt_data['brand-methodology']}")
        ...
        if response.candidates and response.candidates[0].content.parts:
            for part in response.candidates[0].content.parts:
                if hasattr(part, 'inline_data') and part.inline_data:
                    ...
                    brand_slug = prompt_data["brand-methodology"].lower().replace(" ", "-")
  ...
  to incorporate your ACTUAL logo into the CIP mockups.
    ...
    if args.logo:
        ...
        if action == 'generate':
            ...
            print(f"   python ~/.claude/skills/design-assets-guide/scripts/logo/generate.py --brand \"{args.brand}\" --industry \"{args.industry}\"")
