...
description: |
  ...
  Preloaded by dev-backend-engineer, dev-frontend-engineer, dev-fullstack-engineer
  agents.
---
...
3. **Brainstorm Approaches** (for non-trivial tasks)

   **Skip this step if:** single-file change, obvious fix, user gave exact implementation instructions.

   **Do this step if:** multiple valid approaches exist, architectural decision needed, new feature, unfamiliar area.

   Explore 2-3 implementation approaches before writing code:
   ...
   - What breaks if it's wrong? (blast radius)
   - Testing difficulty?

   **Pick the approach** with clear reasoning. Default to simplest unless simplest has obvious drawbacks.
   If genuinely uncertain — present options to user, let them decide.

   This step prevents: over-engineering, wrong abstractions, rework after half-implementation.
...
4. **Analyze & Review Approach**
   ...
   - Reversibility check: Can changes be rolled back without data loss?
     For M/L features: consider feature flags for new behavior.
     For DB migrations: ensure rollback works.
     For external integrations: adapter pattern (easy to swap/disable).
     Principle: Adopt existing code > Adapt existing patterns > Invent new.

   If concerns → discuss with user before proceeding.

**Checkpoint:** List completed preparation steps (including chosen approach if brainstorm was done) before moving to implementation.
...
2. **Write Code**
   ...
   - If tech-spec Complexity Classification is "Product": prefer simplest working solution, use existing patterns. New abstractions require explicit justification.
   - For risky integrations: wrap behind a toggle/config flag if feasible
