...
Run the sync script:
```bash
node .claude/skills/brand-methodology/scripts/sync-brand-to-tokens.cjs
```
...
node .claude/skills/brand-methodology/scripts/inject-brand-context.cjs --json | head -30
...
- `brand-methodology` - Brand context extraction and sync
- `design-system-methodology` - Token generation
