...
2a. **Emit ledger events (observability — best-effort, AFTER the seam returns).** The Workflow engine writes NO `events.jsonl` entries for the pre-gate cascade, so a later observability/analytics reader would otherwise find a near-empty ledger for the engagement. Emit one `phase_completed` per phase the return covers, deriving the payload from the RETURNED object ONLY (never fabricate). Best-effort by design — `ledger-emit.py` exits 0 on any failure, so a failed emit NEVER blocks the cascade:
   ```bash
   # deliver — from r.waves (waveSummaries: merge_ok / strategy / tests per wave)
   python ~/.claude/scripts/ledger-emit.py <engagement> --agent agency-intake --quiet \
     --type phase_completed --phase deliver  --payload-json '{"waveSummaries": <r.waves>}'
   # validate — from r.adversarial_verify + r.validation_files
   python ~/.claude/scripts/ledger-emit.py <engagement> --agent agency-intake --quiet \
     --type phase_completed --phase validate --payload-json '{"adversarial_verify": <r.adversarial_verify>, "validation_files": <r.validation_files>}'
   # gate — from r.gate.failures + r.readyForAcceptance
   python ~/.claude/scripts/ledger-emit.py <engagement> --agent agency-intake --quiet \
     --type phase_completed --phase gate     --payload-json '{"failures": <r.gate.failures>, "readyForAcceptance": <r.readyForAcceptance>}'
   ```
   On a PRE-gate hard-stop (step 3 error contract — no `r.waves`/`r.adversarial_verify`), emit a single `--phase deliver` with `{"error": <r.error>, "failures": <r.gate.failures>, "readyForAcceptance": false}` instead. `<engagement>` is the `engagement/` dir; `--tier <r.tier>` MAY be added.
