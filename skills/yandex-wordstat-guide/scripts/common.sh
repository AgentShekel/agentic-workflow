...
# Methods: topRequests, dynamics, regions, getRegionsTree, userInfo
# Uses curl config file to avoid Windows/Git Bash header escaping issues
wordstat_request() {
    local method="$1"
    local params="$2"

    local ws_url="https://api.wordstat.yandex.net/v1/$method"
    local curl_cfg="${TMPDIR:-/tmp}/ws_curl_cfg_$$.txt"
...
CURLEOF
    ...
    if [[ -n "$params" ]]; then
        echo "-d $params" >> "$curl_cfg"
    fi

    curl -s -K "$curl_cfg" "$ws_url"
    local rc=$?
    rm -f "$curl_cfg"
    return $rc
}
