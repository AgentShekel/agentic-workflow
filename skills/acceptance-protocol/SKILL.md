...
- **Don't write a verdict without binding it.** Every verdict carries `handoff-sha256:` in its `## Iteration {N}` section (`handoff-digest.py` prints the line). An unbound verdict survives the handoff being edited underneath it, which makes the whole acceptance record unfalsifiable. See [verdict-format.md](references/verdict-format.md) §"Bind the verdict to what it judged".
- **Don't edit handoff.md after the verdict and leave the digest alone.** If the edit was immaterial, re-record the digest deliberately. A stale binding is worse than none, because it looks checked.
