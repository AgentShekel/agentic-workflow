...
domain: dev
description: |
  [METHODOLOGY] Post-deploy verification (execute AVP from tech-spec
  on live environment, verify acceptance criteria from user-spec + tech-spec,
  pick up deferred criteria from pre-deploy QA report) using MCP tools
  (Telegram MCP, Playwright, curl, bash). Preloaded by post-deploy-qa agent.
---
...
When Playwright MCP is available and the feature has UI components:

1. **Structured smoke tests** — for each user-facing acceptance criterion:
   - `browser_navigate` to the relevant page
   - `browser_snapshot` to capture DOM state
   - `browser_take_screenshot` for visual evidence
   - Verify key elements exist and contain expected content

2. **Interactive flow verification** — for multi-step user flows:
   - `browser_fill_form` for input fields
   - `browser_click` for buttons/links
   - `browser_wait_for` for async operations
   - Capture before/after screenshots

3. **Responsive checks** (if applicable):
   - `browser_resize` to test breakpoints (mobile: 375px, tablet: 768px, desktop: 1280px)
   - Verify layout doesn't break at each size

4. **Evidence collection**: Save screenshots to `logs/working/screenshots/` with descriptive names. Reference in the QA report JSON.
