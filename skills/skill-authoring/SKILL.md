...
domain: meta
description: |
  [METHODOLOGY] Guide for creating/updating skills with specialized
  knowledge and workflows. Pure reference — invoked via slash command or
  explicit Skill tool call, not auto-triggered by keywords.
---
...
Examples: security-auditing (what to check), testing (when to use which test type), company-info (domain knowledge), database-schemas.
...
│   │   └── domain: (required — one of: marketing, dev, design, meta)
...
**`domain`** (required):
- One of: `marketing`, `dev`, `design`, `meta`
- Three work domains each have one Director skill (`marketing-director`, `dev-director`, `design-director`) that orchestrates leads and specialists in that domain.
- `meta` has no director — system-maintenance utilities.
- Used by `regen_catalog.py` to group skills in `~/.claude/SKILLS.md`
- Used by `lint_skill.py` to detect missing/invalid domain
...
- **Reviews** — code-reviewer, security-auditing, test-reviewer check work with fresh context
...
skill-checker is defined in `~/.claude/agents/skill-checker.md` and has skill-authoring preloaded.
...
- [ ] domain set to valid slug (marketing / dev / design / meta)
- [ ] SKILL.md < 500 lines
...
- [ ] Run portfolio lint: `python ~/.claude/skills/skill-authoring/scripts/lint_skill.py --file <path>/SKILL.md` — fix errors, review warnings
- [ ] Regenerate catalog: `python ~/.claude/skills/skill-authoring/scripts/regen_catalog.py` — refreshes `~/.claude/SKILLS.md`

## Portfolio Hygiene

As the skill portfolio grows, three scripts in `scripts/` keep it navigable and collision-free:

### 1. Before creating a new skill — lint the proposal

Check name uniqueness, trigger-phrase overlap with existing skills, and required frontmatter fields:

```bash
python ~/.claude/skills/skill-authoring/scripts/lint_skill.py \
  --name my-new-skill \
  --domain meta \
  --description "Does X. Use when: 'trigger phrase 1', 'trigger phrase 2'"
```

Exit codes: 0 = clean, 1 = warnings (review), 2 = errors (fix before creating).

### 2. After creating — validate the real file

```bash
python ~/.claude/skills/skill-authoring/scripts/lint_skill.py \
  --file ~/.claude/skills/my-new-skill/SKILL.md
```

### 3. Regenerate catalog

```bash
python ~/.claude/skills/skill-authoring/scripts/regen_catalog.py
```

Reads `domain:` from every `SKILL.md` frontmatter and rewrites the section between `<!-- SKILLS_INDEX:START -->` and `<!-- SKILLS_INDEX:END -->` in `~/.claude/SKILLS.md`. Everything outside the markers is preserved.

### 4. Portfolio-wide health check

Periodically lint every skill to catch drift (missing domain, accidental name collisions, growing trigger overlaps):

```bash
python ~/.claude/skills/skill-authoring/scripts/lint_skill.py --all
```

### When to split into plugins

At 40 skills, flat structure with domain tags works. If the portfolio passes ~100 skills AND you start sharing domain clusters (e.g. publishing marketing-suite), consider extracting into Claude Code plugins. The domain slug becomes the plugin name, trigger becomes `plugin:skill`. Until then, flat + domain-tagged + auto-catalog is lower overhead.
