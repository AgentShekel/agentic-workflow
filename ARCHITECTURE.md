...
| `check-agent-models.py` | Roster-agnostic lint — fails if any agent is missing a `model:` line or declares a tier outside `{opus, sonnet, haiku}`; hardcodes no agent names |
