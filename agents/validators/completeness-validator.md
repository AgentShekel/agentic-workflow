...
- **Complexity guard**: if Complexity Classification section exists and type is "Product" — new abstractions, custom frameworks, or non-standard patterns without explicit justification in Decisions section → severity `critical` (not major). Product features must enforce simplicity: adopt existing patterns > adapt > invent.
...
- **Risks section**: if Risks section has fewer than 2 entries for M/L features → severity `minor`. Risks should not be empty for any feature.
